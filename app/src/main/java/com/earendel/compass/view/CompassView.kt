...
import com.earendel.compass.databinding.CompassViewBinding
...
class CompassView(context: Context, attributes: AttributeSet) : ConstraintLayout(context, attributes) {
    ...
    fun setAzimuth(value: Float) {
        ...
        // Hard-code strictly White in Dark mode and Black in Light mode
        val textColorPrimary = ContextCompat.getColor(context, if (isLightTheme) R.color.pure_black else R.color.pure_white)
        val backgroundColor = ContextCompat.getColor(context, if (isLightTheme) R.color.pure_white else R.color.pure_black)

        binding.degree0Text.setTextColor(textColorPrimary)

        if (azimuth.degrees >= 359f || azimuth.degrees <= 1f) {
            binding.cardinalDirectionNorthText.setTextColor(ContextCompat.getColor(context, R.color.pure_white))
            binding.deviceHeadingIndicator.colorFilter = PorterDuffColorFilter(ContextCompat.getColor(context, android.R.color.holo_red_dark), PorterDuff.Mode.SRC_IN)
        } else {
    ...
    private fun highlightCardinalDirections(azimuth: Azimuth) {
        ...
        updateCardinalHighlight(binding.cardinalDirectionNorthText, azimuth.degrees >= 359f || azimuth.degrees <= 1f, true)
    ...
    private fun updateCardinalHighlight(textView: TextView, active: Boolean, isNorth: Boolean) {
        ...
        val pureBlack = ContextCompat.getColor(context, R.color.pure_black)
        
        if (active) {
            val bgColor = if (isNorth) {
                ContextCompat.getColor(textView.context, android.R.color.holo_red_dark)
            } else {
                if (isLightTheme) pureBlack else pureWhite
            }
            
            // Draw text in theme background color (inverted)
            val textColor = if (isNorth) pureWhite else (if (isLightTheme) pureWhite else pureBlack)
            ...
            textView.setTextColor(if (isLightTheme) pureBlack else pureWhite)
        }
    }
    ...
    private fun updateHeadingIndicatorColor(azimuth: Azimuth) {
        ...
        val color = if (azimuth.degrees >= 359f || azimuth.degrees <= 1f) {
            ...
            if (isLightTheme) ContextCompat.getColor(context, R.color.pure_black) 
            else ContextCompat.getColor(context, R.color.pure_white)
        }
        binding.deviceHeadingIndicator.setColorFilter(color, PorterDuff.Mode.SRC_IN)
    }
